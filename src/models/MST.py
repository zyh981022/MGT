...
class Mutil_Scale_Attention(nn.Module):
    ...
    def forward(self,x, mask=None, dropout=None):
        ...
        for h_i in range(self.local_num):
            ...
            kv_shunted_win_i, window_mask_i = local_reconstruction(k_l_i,window_size=h_i+1,attn_mask=mask,pool_method=self.pool_method, pool_layer=self.pool_layers[h_i])  # window_size = [2,3,4]  return [2, b, num_window, d_k]
