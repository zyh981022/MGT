...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, d_model, d_ff, dropout=0.1, activate="gelu",args=None):
        super(PositionwiseFeedForward, self).__init__()
        self.w_1 = nn.Linear(d_model, d_ff)
        self.w_2 = nn.Linear(d_ff, d_model)
        self.dropout = nn.Dropout(dropout)
        self.activation = GELU() if activate=="gelu" else nn.ReLU()
        self.MLP_DS = args.MLP_DS
        if self.MLP_DS != None and self.MLP_DS == 'DS':
    ...
    def forward(self, x):
        if self.MLP_DS == 'DS':
            #print('add DS')
            x = self.w_1(x)
            return self.w_2(self.activation(self.dropout(x+self.dwconv(x))))
        else:
...
class TokenEmbedding(nn.Embedding):
    def __init__(self, vocab_size, embed_size=512): # num_items+2
        super().__init__(vocab_size, embed_size, padding_idx=vocab_size-1, ) # 填充的不学习
...
class PositionalEmbedding(nn.Module):
    def __init__(self, max_len, d_model, dropout=0):
        ...
        self.pe = nn.Embedding(max_len+3, d_model, padding_idx=-1) # [max_len: [sep], max_len+1: [end], max_len+2: [pad]]   -1不学习，都为0
...
class BERTEmbedding(nn.Module):
    ...
    def __init__(self, vocab_size, embed_size, max_len, dropout=0.1, pos_dropout=0):
        super().__init__()
        self.token = TokenEmbedding(vocab_size=vocab_size, embed_size=embed_size) # num_items+2
